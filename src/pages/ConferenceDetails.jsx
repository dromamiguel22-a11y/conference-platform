...
function ConferenceDetails({ registeredIds, onRegister, selectedSessions, onAddSession, onRemoveSession, isDarkMode }) {
  ...
  if (!conference) {
    return (
      <div className="min-h-screen p-6" style={decoBgStyle(theme)}>
        ...
        <Link to="/" style={{ color: crimson }} className="hover:underline">← Back to Home</Link>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pb-16" style={decoBgStyle(theme)}>
        ...
        <div className="animate-fade-in-up ziggurat-frame inline-block max-w-3xl" style={{ '--frame-color': theme.accent }}>
          <div className="ziggurat-inner" style={{ backgroundColor: theme.panel }}>
            <div className="px-8 py-10 md:px-14 md:py-14 flex flex-col items-center gap-5">
              ...
              <h1 className="font-['Bodoni_Moda'] text-3xl md:text-5xl uppercase leading-tight" style={{ color: crimson }}>
                {conference.title}
              </h1>
              ...
              <button
                ...
                className="mt-4 px-10 py-4 font-['Montserrat'] text-xs uppercase tracking-widest font-bold transition-all duration-300 border-2 rounded-sm"
                style={
                  isRegistered
                    ? { backgroundColor: theme.panelAlt, color: theme.muted, borderColor: theme.border, cursor: 'default' }
                    : { backgroundColor: crimsonBright, color: '#fff', borderColor: theme.accent }
                }
                onMouseEnter={(e) => { if (!isRegistered) e.currentTarget.style.transform = 'scale(1.05)'; }}
      ...
      <section className="px-6 max-w-5xl mx-auto mb-14">
        <h2 className="font-['Bodoni_Moda'] text-2xl text-center mb-10 uppercase tracking-widest" style={{ color: crimson }}>
            ...
            >
              <div className="relative w-24 h-24 mb-3 transition-transform duration-300 group-hover:scale-110">
                ...
                <div className="absolute inset-1.5 rounded-full border flex items-center justify-center transition-shadow duration-300" style={{ borderColor: theme.accent, backgroundColor: theme.panelAlt }}>
                  <span className="font-['Bodoni_Moda'] text-xl" style={{ color: crimson }}>{speaker.name.charAt(0)}</span>
                </div>
              </div>
              <h3 className="font-['Montserrat'] text-sm font-bold uppercase" style={{ color: theme.text }}>{speaker.name}</h3>
              <p className="text-sm font-['Montserrat'] mt-1 leading-snug" style={{ color: crimson }}>{speaker.topic}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="px-6 max-w-4xl mx-auto">
        <div className="ziggurat-frame" style={{ '--frame-color': theme.accent }}>
          <div className="ziggurat-inner p-6 md:p-10" style={{ backgroundColor: theme.panel }}>
            <h2 className="font-['Bodoni_Moda'] text-xl text-center mb-10 uppercase tracking-[0.15em] border-b pb-5" style={{ color: crimson, borderColor: theme.border }}>
            ...
            <div className="flex flex-col gap-6">
              {conference.agenda.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <div className="font-['Bodoni_Moda'] text-lg w-20 shrink-0" style={{ color: crimson }}>
                      {item.time}
                    </div>
